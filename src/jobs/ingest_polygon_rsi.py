...
def run(run_date: str, window: int = DEFAULT_WINDOW, limit: int = DEFAULT_LIMIT):
    print(f"Starting S&P 500 RSI ingestion for {run_date} (window={window}, limit={limit})...")
    ...
    print(f"Successfully wrote {len(df)} RSI data points to {fq_table} for date {run_date}")
    session.close()
